...
(function () {
  ...
  function extensionBadge(dateKey, className = 'week-extension-mark') {
    return window.LumaExtensions?.badges(dateKey,className) || '';
  }
  ...
  function weekHolidaySubline(dateKey, lunar) {
    ...
    const extension = extensionBadge(dateKey);
    if (!caption) {
      const lunarText = lunar ? `<span class="week-col-lunar">${lunar}</span>` : '';
      return `<span class="week-col-subline">${lunarText}${extension}</span>`;
    }
    ...
    return `<span class="week-col-subline">${holiday}${extension}</span>`;
  }
  let view = 'month';
  ...
  function renderMini(host, keys) {
    ...
    for (let index = 0; index < 42; index += 1) {
      ...
      const extensionMini = window.LumaExtensions?.badges(key,'week-mini-extension',true) || '';
      cells.push(`<button type="button" class="week-mini-day${muted ? ' is-muted' : ''}${weekSet.has(key) ? ' is-week' : ''}${key === today ? ' is-today' : ''}${key === anchorKey ? ' is-anchor' : ''}${holidayClass}" data-date="${key}"${title}${aria}>${date.getDate()}${extensionMini}${workdot}</button>`);
    }
  ...
  function renderBoard() {
    ...
    const keys = weekKeys(anchorKey);
    window.LumaExtensions?.ensureRange?.(keys[0], keys[6]);
